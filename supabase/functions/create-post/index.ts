...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return await withIdempotency(req, supabase, user.id, "create-post", async () => {
      ...
      if (media_urls.length > 0) {
        if (!media_type || (media_type !== "photo" && media_type !== "video")) {
          throw new AppError(
            "VALIDATION_ERROR",
            "media_type is required when media_urls is present and must be 'photo' or 'video'",
            400,
          );
        }
        if (media_urls.length > 10) {
          throw new AppError(
            "VALIDATION_ERROR",
            "media_urls exceeds maximum limit of 10 items",
            400,
          );
        }
        ...
        for (const url of media_urls) {
          if (
            !url.startsWith(supabaseUrl) || !url.includes("/storage/v1/object/public/post-media/")
          ) {
            throw new AppError(
              "INVALID_MEDIA_URL",
              "Media URL must be a valid Supabase Storage URL in post-media bucket",
              400,
            );
          }
        }
      ...
      if (tags.length > 0) {
        ...
        for (const t of tags) {
          if (typeof t !== "string" || t.length > 30) {
            throw new AppError(
              "VALIDATION_ERROR",
              "Each tag must be a string, max 30 characters",
              400,
            );
          }
        }
