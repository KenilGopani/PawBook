...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return await withIdempotency(req, supabase, user.id, "send-friend-request", async () => {
      ...
      if (existing?.rel_type === "FRIEND") {
        throw new AppError("ALREADY_FRIENDS", "Already friends", 409);
      }
      if (existing?.rel_type === "BLOCKED") throw new AppError("BLOCKED", "Blocked", 403);
      if (existing?.rel_type === "FRIEND_REQ") {
        throw new AppError("REQUEST_PENDING", "Request already pending", 409);
      }
