...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Validation
    if (!pet_id || last_seen_lat === undefined || last_seen_lng === undefined || !last_seen_at || !description || !contact_info) {
      throw new AppError("VALIDATION_ERROR", "pet_id, last_seen_lat, last_seen_lng, last_seen_at, description, and contact_info are required", 400);
    }

    if (description.length < 10 || description.length > 500) {
      throw new AppError("VALIDATION_ERROR", "description must be between 10 and 500 characters", 400);
    }
    ...
    if (lastSeenDate.getTime() < sevenDaysAgo || lastSeenDate.getTime() > Date.now() + 60 * 1000) {
