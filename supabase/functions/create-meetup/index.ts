...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return await withIdempotency(req, supabase, user.id, "create-meetup", async () => {
      ...
      if (
        !Array.isArray(invited_pet_ids) || invited_pet_ids.length < 1 || invited_pet_ids.length > 9
      ) {
        throw new AppError(
          "VALIDATION_ERROR",
          "invited_pet_ids must be an array of 1 to 9 pet IDs",
          400,
        );
      }
      ...
      if (!is_group && invited_pet_ids.length !== 1) {
        throw new AppError(
          "VALIDATION_ERROR",
          "1-on-1 meetups must have exactly 1 invited pet",
          400,
        );
      }
      ...
      for (const petId of invited_pet_ids) {
        ...
        if (blocked) {
          throw new AppError(
            "BLOCKED",
            `Pet ${petId} has a blocked relationship with the organizer pet`,
            403,
          );
        }
      }
